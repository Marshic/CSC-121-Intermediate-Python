...
'''
animals is a dictionary and each item in animals is also a dictionary where the key is "Big Cats" and the value is another dictionary. Now the keys are Lion, Tiger and Cheetah and their values 
are lists.
...
# Retrieve the string "Greenland Polarbear" with an f string
...
# Retrieve the Queen Angelfish from fish_types
print(fish_types[2][2][1][2])
'''
for f in fish_types:
    print(f)
    for i in fish_types[f]:
        print(fish_types[i])
        for s in fish_types[i]:
            print(fish_types[s])
'''
...
for key in dog_breeds:
    print(key)
    for inner_key in dog_breeds[key]:
